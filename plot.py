...
CHAPTERS = [
 dict(n=1, part=1, construct="SELECT / WHERE", tables=["police_report", "interview"], answer_key="report_id",
  ...
  discovery_compete=[dict(query="SELECT transcript FROM interview WHERE person_name = 'the Meurice night manager'", must_contain="19120611")],
  objective_compete="Find the police report for the burglary at the Hotel Meurice. Read the night manager's statement in interview -- it gives the exact night; police_report dates are written as YYYYMMDD.",
 ...
 dict(n=2, part=1, construct="ORDER BY / LIMIT", tables=["hotel_register"], answer_key="neighbour",
  ...
  objective_compete="Ganimard says the report from chapter 1 already told you which floor and which price rank to look for. Re-read it, then in hotel_register find that guest's SUITE NUMBER for the night of 17 May 1912 (checkin <= 19120517 and checkout > 19120517).",
 ...
 dict(n=3, part=1, construct="LIKE", tables=["cab_ride"], answer_key="plate",
  ...
  objective_compete="A rival cab company keeps its own book. Read its despatcher's statement in interview, then find in cab_ride the ride that dropped a fare at Gare Saint-Lazare that night matching what he said.",
 ...
 dict(n=4, part=1, construct="GROUP BY / HAVING", tables=[], answer_key="fence_address",
  ...
  discovery_compete=[dict(query="SELECT dropoff, COUNT(*) AS n FROM cab_ride WHERE plate = '{compete_plate}' AND date BETWEEN 19120513 AND 19120519 GROUP BY dropoff ORDER BY n DESC", must_contain="{compete_fence_address}")],
  objective_compete="In cab_ride, for the plate you found in chapter 3, between 19120513 and 19120519, look at everywhere it went -- count the visits per address -- and find the one it kept returning to.",
 ...
 dict(n=5, part=1, construct="JOIN", tables=["person", "address"], answer_key="fence",
  ...
  discovery_compete=[
    dict(query="SELECT id FROM address WHERE number = {compete_fence_number} AND street = '{compete_fence_street}'", row_count=1),
    dict(query="SELECT name, occupation FROM person WHERE address_id = (SELECT id FROM address WHERE number = {compete_fence_number} AND street = '{compete_fence_street}')", row_count=5),
  ],
  objective_compete="First find the id of the address from chapter 4 in the address table, then list who lives there (person.address_id equal to that id) and pick out the one whose occupation is jeweller. Only once you have done that by hand should you write it as one JOIN.",
 ...
 dict(n=6, part=1, construct="JOIN x3 + SUM", tables=["bank_account", "bank_transaction"], answer_key="shell_account",
  ...
  discovery_compete=[
    ...
    dict(query="SELECT counterparty_id, SUM(amount) AS total FROM bank_transaction WHERE account_id = (SELECT id FROM bank_account WHERE person_id = (SELECT id FROM person WHERE name = '{compete_fence_name}')) AND date BETWEEN 19120501 AND 19120531 GROUP BY counterparty_id", must_contain="{compete_shell_account}"),
  ],
  objective_compete="First find the jeweller's own id in bank_account (person_id is the jeweller from chapter 5), then look at everything bank_transaction paid out of it in May 1912 (date between 19120501 and 19120531) and total it by counterparty. Only then write the whole thing as one join.",
 ...
 dict(n=7, part=1, construct="CASE WHEN", tables=["telegram"], answer_key="night_telegram_id",
  ...
  discovery_compete=[dict(query="SELECT time FROM telegram WHERE office = 'Bourse' AND date = {theft_date} ORDER BY time", must_contain="1930")],
  objective_compete="The Bourse office also sent wires that day. Look at the raw times first, then classify them the same way, and find the single one sent in the evening (18:00 or later).",
 ...
 dict(n=8, part=1, construct="RANK() OVER + subquery", tables=["room_service"], answer_key="lupin_alias",
  ...
  objective_compete="Read the whole telegram from chapter 7 -- it names what he always drinks before the market opens. Then using room_service for 18 May, rank each suite's orders by time and find the suite whose SECOND order (rank 2) matches that, before 06:00.",
  answer_form_compete="the suite number",
